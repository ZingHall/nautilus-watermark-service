...
# Build a musl-compatible Rust nightly installation
...
# Copy Rust toolchain (musl-compatible)
...
FROM base as build
COPY . .
# Remove --locked to allow dependency updates for sui-sdk-types compatibility
...
# Build without feature flags since they've been removed from Cargo.toml
